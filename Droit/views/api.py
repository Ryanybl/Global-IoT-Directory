...
from ..utils import get_target_url, is_json_request, clean_thing_description, add_policy_to_storage, delete_policy_from_storage, is_policy_request, is_request_allowed
...
def policy():
    ...
    policy['uid'] = str(uuid.uuid4())
    if not is_policy_request(policy, ["description", "effect", "rules", "targets", "priority"]):
        return jsonify(ERROR_POLICY), 400
    
    if add_policy_to_storage(policy, json['location']):
